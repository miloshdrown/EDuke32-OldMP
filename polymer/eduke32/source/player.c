...
void P_ProcessInput(int snum)
{
    ...
    X_OnEvent(EVENT_PROCESSINPUT, p->i, snum, -1);
...
//    if ((sb_snum&(1<<2)))
    {
        Gv_SetVar(g_iReturnVarID,0,p->i,snum);
        X_OnEvent(EVENT_PRESSEDFIRE, p->i, snum, -1);
        if (Gv_GetVar(g_iReturnVarID,p->i,snum) != 0)
            sb_snum &= ~BIT(SK_FIRE);
    }
    ...
    ActorExtra[p->i].floorz = fz;
    ActorExtra[p->i].ceilingz = cz;
    ...
    if (TEST_SYNC_KEY(sb_snum, SK_LOOK_LEFT))
    {
        // look_left
        Gv_SetVar(g_iReturnVarID,0,p->i,snum);
        X_OnEvent(EVENT_LOOKLEFT,p->i,snum, -1);
        if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
        {
            p->look_ang -= 152;
            p->rotscrnang += 24;
    ...
    if (TEST_SYNC_KEY(sb_snum, SK_LOOK_RIGHT))
    {
        // look_right
        Gv_SetVar(g_iReturnVarID,0,p->i,snum);
        X_OnEvent(EVENT_LOOKRIGHT,p->i,snum, -1);
        if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
        {
            p->look_ang += 152;
            p->rotscrnang -= 24;
    ...
    */
    ...
    if (psectlotag == 2)
    {
        ...
        {
            Gv_SetVar(g_iReturnVarID,0,p->i,snum);
            X_OnEvent(EVENT_SWIMUP,p->i,snum, -1);
            if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
            {
                // jump
        ...
        {
            Gv_SetVar(g_iReturnVarID,0,p->i,snum);
            X_OnEvent(EVENT_SWIMDOWN,p->i,snum, -1);
            if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
            {
                // crouch
                if (p->poszv < 0) p->poszv = 0;
    ...
    else if (p->jetpack_on)
    {
        ...
        if (TEST_SYNC_KEY(sb_snum, SK_JUMP))         //A (soar high)
        {
            // jump
            Gv_SetVar(g_iReturnVarID,0,p->i,snum);
            X_OnEvent(EVENT_SOARUP,p->i,snum, -1);
            if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
            {
                p->posz -= j;
                p->crack_time = 777;
        ...
        if (TEST_SYNC_KEY(sb_snum, SK_CROUCH))   //Z (soar low)
        {
            // crouch
            Gv_SetVar(g_iReturnVarID,0,p->i,snum);
            X_OnEvent(EVENT_SOARDOWN,p->i,snum, -1);
            if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
            {
                p->posz += j;
                p->crack_time = 777;
        ...
        if (p->posz < (ActorExtra[p->i].ceilingz+(18<<8)))
            p->posz = ActorExtra[p->i].ceilingz+(18<<8);
    ...
    else if (psectlotag != 2)
    {
            ...
            else
            {
                ...
                if ((p->posz+p->poszv) >= (fz-(i<<8)) && p->cursectnum >= 0)   // hit the ground
                    if (sector[p->cursectnum].lotag != 1)
                    {
                        ...
                        else if (p->poszv > 2048) A_PlaySound(DUKE_LAND,p->i);
                    }
            }
        }
        ...
        else
        {
            ...
            if (TEST_SYNC_KEY(sb_snum, SK_CROUCH))
            {
                // crouching
                Gv_SetVar(g_iReturnVarID,0,p->i,snum);
                X_OnEvent(EVENT_CROUCH,p->i,snum, -1);
                if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
                {
                    p->posz += (2048+768);
                    p->crack_time = 777;
            ...
            else if (TEST_SYNC_KEY(sb_snum, SK_JUMP) && p->jumping_toggle == 0)
            {
                if (p->jumping_counter == 0)
                    ...
                    {
                        Gv_SetVar(g_iReturnVarID,0,p->i,snum);
                        X_OnEvent(EVENT_JUMP,p->i,snum, -1);
                        if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
                        {
                            p->jumping_counter = 1;
                            p->jumping_toggle = 1;
    ...
    if (g_player[snum].sync->extbits&(1))
        X_OnEvent(EVENT_MOVEFORWARD,p->i,snum, -1);

    if (g_player[snum].sync->extbits&(1<<1))
        X_OnEvent(EVENT_MOVEBACKWARD,p->i,snum, -1);

    if (g_player[snum].sync->extbits&(1<<2))
        X_OnEvent(EVENT_STRAFELEFT,p->i,snum, -1);

    if (g_player[snum].sync->extbits&(1<<3))
        X_OnEvent(EVENT_STRAFERIGHT,p->i,snum, -1);

    if (g_player[snum].sync->extbits&(1<<4) || g_player[snum].sync->avel < 0)
        X_OnEvent(EVENT_TURNLEFT,p->i,snum, -1);

    if (g_player[snum].sync->extbits&(1<<5) || g_player[snum].sync->avel > 0)
        X_OnEvent(EVENT_TURNRIGHT,p->i,snum, -1);
...
HORIZONLY:
    ...
    if (ud.clipping == 0)
        j = (pushmove(&p->posx,&p->posy,&p->posz,&p->cursectnum,164L,(4L<<8),(4L<<8),CLIPMASK0) < 0 && A_GetFurthestAngle(p->i,8) < 512);
    else j = 0;

    if (ud.clipping == 0)
    {
        if (klabs(ActorExtra[p->i].floorz-ActorExtra[p->i].ceilingz) < (48<<8) || j)
        {
    ...
    {
        Gv_SetVar(g_iReturnVarID,0,p->i,snum);
        X_OnEvent(EVENT_RETURNTOCENTER,p->i,snum, -1);
        if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
        {
    ...
    if (TEST_SYNC_KEY(sb_snum, SK_LOOK_UP))
    {
        // look_up
        Gv_SetVar(g_iReturnVarID,0,p->i,snum);
        X_OnEvent(EVENT_LOOKUP,p->i,snum, -1);
        if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
        {
            p->return_to_center = 9;
    ...
    else if (TEST_SYNC_KEY(sb_snum, SK_LOOK_DOWN))
    {
        // look_down
        Gv_SetVar(g_iReturnVarID,0,p->i,snum);
        X_OnEvent(EVENT_LOOKDOWN,p->i,snum, -1);
        if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
        {
            p->return_to_center = 9;
    ...
    else if (TEST_SYNC_KEY(sb_snum, SK_AIM_UP))
    {
        // aim_up
        Gv_SetVar(g_iReturnVarID,0,p->i,snum);
        X_OnEvent(EVENT_AIMUP,p->i,snum, -1);
        if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
        {
            // running
    ...
    else if (TEST_SYNC_KEY(sb_snum, SK_AIM_DOWN))
    {
        // aim_down
        Gv_SetVar(g_iReturnVarID,0,p->i,snum);
        X_OnEvent(EVENT_AIMDOWN,p->i,snum, -1);
        if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
        {
            // running
            if (TEST_SYNC_KEY(sb_snum, SK_RUN)) p->horiz -= 6;
    ...
    if (p->knee_incs > 0)
    {
        ...
        if (p->knee_incs > 15)
        {
            ...
            if (p->actorsqu >= 0 && dist(&sprite[p->i],&sprite[p->actorsqu]) < 1400 && sprite[p->actorsqu].statnum != MAXSTATUS)
            {
...
SHOOTINCODE:
    if (TEST_SYNC_KEY(sb_snum, SK_HOLSTER))   // 'Holster Weapon
    {
        ...
        Gv_SetVar(g_iWeaponVarID,p->curr_weapon,p->i,snum);
        Gv_SetVar(g_iWorksLikeVarID,aplWeaponWorksLike[p->curr_weapon][snum],p->i,snum);
        X_OnEvent(EVENT_HOLSTER, p->i, snum, -1);
        if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
        {
             ...
             p->last_weapon == -1 && (p->weapon_pos == 0 || p->holster_weapon == 1))
    {
        ...
        else
        {
            ...
            Gv_SetVar(g_iWeaponVarID,p->curr_weapon,p->i,snum);
            Gv_SetVar(g_iWorksLikeVarID,aplWeaponWorksLike[p->curr_weapon][snum],p->i,snum);
            X_OnEvent(EVENT_FIRE, p->i, snum, -1);
            if (Gv_GetVar(g_iReturnVarID,p->i,snum) == 0)
            {
