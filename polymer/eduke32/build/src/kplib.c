...
static int xsizbpl, ixsiz, ixoff, iyoff, ixstp, iystp, intlac, nbpl;
B_KPLIB_STATIC int ATTRIBUTE((used)) trnsrgb ASMNAME("trnsrgb");
...
B_KPLIB_STATIC unsigned char olinbuf[65536] ASMNAME("olinbuf"); //WARNING:max xres is: 65536/bpp-1
static int gotcmov = -2;
B_KPLIB_STATIC int ATTRIBUTE((used)) abstab10[1024] ASMNAME("abstab10");
...
static int kpcxrend(const char *buf, int fleng,
                    ...
                    int daglobxoffs, int daglobyoffs)
{
    int  j, x, y, nplanes, x0, x1, y0, y1, xsiz, ysiz;
...
{
    int x = 0, y = 0, z = 0, xx, yy, xsiz, ysiz, dxt, al[2], ai, k, v, c0, c1, stride;
...
intptr_t kzopen(const char *filnam)
{
    ...
    if (kzcheckhash(filnam,&zipnam,&zipseek))
    {
        ...
        {
        case 0:
            kzfs.i = 0; return((intptr_t)kzfs.fil);
        case 8:
            if (!pnginited) { pnginited = 1; initpngtables(); }
            kzfs.comptell = 0;
